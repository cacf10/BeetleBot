...
def socket_connect():	 #Call this function to connect with the server
	...
	if ip_adr == '':	  #If no input IP address in Entry,import a default IP
		...
		l_ip_4.config(text='连接中')
		l_ip_4.config(bg='#FF8F00')
		l_ip_5.config(text='默认IP:%s'%ip_adr)
		pass
	...
	for i in range (1,6): #Try 5 times if disconnected
		#try:
		if ip_stu == 1:
			print("正在连接服务 @ %s:%d..." %(SERVER_IP, SERVER_PORT))
			print("正在连接中...")
			...
			print("连接失败，请稍后再试！")
			l_ip_4.config(text='第 %d/5 次 重连'%i)
...
def loop():					  #GUI
	...
	while True:
		...
		root.title('BeetleBot Arduino 机器人')	  #Main window title
		...
		l_ip=tk.Label(root,width=18,text='状态',fg=color_text,bg=color_btn)
		l_ip.place(x=30,y=110)						   #Define a Label and put it in position

		l_ip_4=tk.Label(root,width=18,text='连接已关闭',fg=color_text,bg='#F44336')
		l_ip_4.place(x=218,y=110)						 #Define a Label and put it in position

		l_ip_5=tk.Label(root,width=18,text='使用默认IP',fg=color_text,bg=color_btn)
		...
		l_ip_3=tk.Label(root,width=10,text='IP 地址:',fg=color_text,bg='#000000')
		l_ip_3.place(x=143,y=15)						 #Define a Label and put it in position

		Btn_Steady = tk.Button(root, width=8, text='稳定运行',fg=color_text,bg=color_btn,relief='ridge')
		Btn_Steady.place(x=284,y=195)
		Btn_Steady.bind('<ButtonPress-1>', call_steady)
		root.bind('<KeyPress-f>', call_steady)

		Btn_Ultra = tk.Button(root, width=8, text='超声波',fg=color_text,bg=color_btn,relief='ridge')
		Btn_Ultra.place(x=284,y=230)
		Btn_Ultra.bind('<ButtonPress-1>', call_ultra)
		root.bind('<KeyPress-x>', call_ultra)

		Btn0 = tk.Button(root, width=8, text='前进',fg=color_text,bg=color_btn,relief='ridge')
		Btn1 = tk.Button(root, width=8, text='后退',fg=color_text,bg=color_btn,relief='ridge')
		Btn2 = tk.Button(root, width=8, text='向左',fg=color_text,bg=color_btn,relief='ridge')
		Btn3 = tk.Button(root, width=8, text='向右',fg=color_text,bg=color_btn,relief='ridge')

		Btn_LeftSide = tk.Button(root, width=8, text='自动',fg=color_text,bg=color_btn,relief='ridge')
		Btn_LeftSide.place(x=30,y=195)
		Btn_LeftSide.bind('<ButtonPress-1>', call_auto)

		Btn_RightSide = tk.Button(root, width=8, text='进攻',fg=color_text,bg=color_btn,relief='ridge')
		...
		Btn14= tk.Button(root, width=8,height=2, text='连接',fg=color_text,bg=color_btn,command=connect_click,relief='ridge')
